...
kittensForm.addEventListener('submit', function(event){
  event.preventDefault();
  console.log(event);
  var catNameValue = event.target.catName.value;
  console.log(catNameValue);

  var likesValue = event.target.likes.value.split(',');
  ...
  var imagePaht = `images/${catNameValue}.jpeg`;

 // (name, likes, imagePath, goodWithKids, goodWithDogs, goodWithOtherCats, breed)
  var newCat = new Cat(catNameValue,likesValue, imagePaht,isGoodWithKids,isGoodWithDogs, isGoodWithCats);
  cats.push(newCat);
